...
# from rayoptics.elem.surface import DecenterData, Circular, Rectangular, Elliptical
...
def read_lens_url(url, **kwargs) -> tuple[ComponentGroup, tuple[dict, dict]]:
...
def read_lens(filename, inpt, **kwargs) -> tuple[ComponentGroup, tuple[dict, dict]]:
...
def process_line(opt_model, line, line_no):
    ...
    # WWGT 1 1 1
        ...
        sr = osp.spectral_region
        sr.wavelengths = [float(i) * 1e3 for i in inputs.split() if i]
    elif cmd == "WWGT":
        sr = osp.spectral_region
        sr.spectral_wts = [float(i) * 1e3 for i in inputs.split() if i]
...
def handle_types_and_params(optm, cur, cmd, inputs):
    ...
    if cmd == "TYPE":
        ...
        if typ == "EVENASPH":
            ...
            new_profile = profiles.mutate_profile(cur_profile, "EvenPolynomial")
            ...
            cur_profile = ifc.profile
            new_profile = profiles.mutate_profile(cur_profile, "RadialPolynomial")
            ifc.profile = new_profile
        elif typ == "COORDBRK":
...
def field_spec_data(optm, cmd, inputs):
    ...
    if len(fov.fields) < len(inputs):
        fov.fields += [Field(fov=fov) for f in range(len(fov.fields), len(inputs))]
